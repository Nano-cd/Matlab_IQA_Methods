...
train_data = features(1:2000,:);
train_mos = mos_new(1:2000,:);

test_data = features(2001:4000,:);
test_mos = mos_new(2001:4000,:);
svr_model = svmtrain(train_mos, train_data, cmd);
[pred_mos, ~, ~] = svmpredict(test_mos, test_data, svr_model);
save('pred_mos.mat','pred_mos');
save('test_mos.mat','test_mos');
% test_mos=mapminmax('reverse',test_mos',ps);
% pred_mos=mapminmax('reverse',pred_mos',ps);
pred_mos = pred_mos';
test_mos = test_mos';
